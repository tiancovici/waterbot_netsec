...
int     smartPlug_pw_callback (char *buf, int size, int rwflag, void *userdata)
{
    strncpy(buf, pw, size);
    buf[size - 1] = '\0';
    return strlen(buf);
...
int main(void)
{
  ...
  mosquitto_tls_opts_set(p_mosq,
    ...
    mosquitto_tls_set(p_mosq, 
            ...
            "/etc/mosquitto/certs",
            "/etc/mosquitto/client2.crt",
            "/etc/mosquitto/client2.key",
            &smartPlug_pw_callback);
